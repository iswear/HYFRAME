...
hy.action.Manager.prototype.addActionBinder = function(sprite,action,target,callBack,loop){
    var actionBinder = new hy.action.Binder({sprite:sprite,action:action,target:target,callBack:callBack,loop:loop});
    this._actionBinders.push(actionBinder);
}
hy.action.Manager.prototype.removeAllActions = function(){
    for(var i=this._actionBinders.length-1;i>=0;--i){
        var actionBinder = this._actionBinders[i];
        actionBinder.setRemoveFlag(true);
    }
}
...
hy.action.Manager.prototype.runActions = function(deltaTime){
    if(!this._paused){
        for(var i=this._actionBinders.length-1;i>=0;--i){
            var actionBinder = this._actionBinders[i];
            if(!actionBinder.getRemoveFlag()){
                var result = actionBinder.execute(deltaTime);
                actionBinder.setRemoveFlag(result && !actionBinder.getLoop());
            }else{
...
hy.action.Binder.prototype.init = function(config){
    this.superCall("init",[config]);
    this._sprite = this.isUndefined(config.sprite) ? null : config.sprite;
    this._action = this.isUndefined(config.action) ? null : config.action;
    this._target = this.isUndefined(config.target) ? null : config.target;
    this._callBack = this.isUndefined(config.callBack) ? null : config.callBack;
    this._loop = this.isUndefined(config.loop) ? false : config.loop;
    this._runParams = {};
...
hy.action.Binder.prototype.setRemoveFlag = function(removeFlag){
    this._removeFlag = removeFlag;
}
hy.action.Binder.prototype.getRemoveFlag = function(){
...
hy.action.Scheduler.prototype.init = function(config){
    ...
    this._target = this.isUndefined(config.target) ? null : config.target;
    this._callBack = this.isUndefined(config.callBack) ? null : config.callBack;
    this._interval = this.isUndefined(config.interval) ? 0 : config.interval;
    this._repeats = this.isUndefined(config.repeats) ? 0 : config.repeats;
    this._param = this.isUndefined(config.param) ? null : config.param;
};
...
hy.action.List = hy.extend(hy.action.Base);
hy.action.List.prototype.init = function(config){
    this.superCall("init",[config]);
    this._actions = this.isUndefined(config.actions) ? [] : config.actions;
}
hy.action.List.prototype.setActions = function(actions){
    this._actions = actions;
}
hy.action.List.prototype.getActions = function(){
    return this._actions;
}

hy.action.SyncList = hy.extend(hy.action.List);
hy.action.SyncList.prototype.execute = function(actionBinder, deltaTime){
    var initFlag = actionBinder.getRunParams("inited");
    var actionBinders;
    if(!initFlag){
        actionBinders = [];
...
hy.action.AsyncList = hy.extend(hy.action.List);
hy.action.AsyncList.prototype.execute = function(actionBinder, deltaTime){
    var initFlag = actionBinder.getRunParams("inited");
    var actionBinders;
    if(!initFlag){
        actionBinders = [];
