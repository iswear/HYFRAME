...
HY.Core.Action.DELMode.CLEANNONE = 0;
HY.Core.Action.DELMode.CLEANTHIS = 1;
HY.Core.Action.DELMode.CLEANALL = 2;
HY.Core.Action.DELMode.CLEANSAMEGROUP = 3;
HY.Core.Action.DELMode.CLEANDIFFGROUP = 4;
...
HY.Core.Action.Manager.prototype.addActionLink = function(sprite,action,clearMode,childClean){
    if(clearMode != undefined){
		switch (clearMode){
			case HY.Core.Action.DELMode.CLEANALL:
			{
				this.removeAllActionInSprite(sprite,childClean);
				break;
			}
			case HY.Core.Action.DELMode.CLEANTHIS:
			{
				this.removeActionInSprite(sprite,action,childClean);
				break;
			}
			case HY.Core.Action.DELMode.CLEANDIFFGROUP:
			{
				this.removeActionInSpriteWithoutGroup(sprite,action.getGroup(),childClean);
				break;
			}
			case HY.Core.Action.DELMode.CLEANSAMEGROUP:
			{
				this.removeActionInSpriteWithGroup(sprite,action.getGroup(),childClean);
				break;
			}
			default :
			{
...
HY.Core.Action.Manager.prototype.removeActionInSpriteWithGroup = function(sprite,group,childRemoved){
	var i,j;
	for( i = this._actionLinkList.length-1;i>=0;--i){
		var curActionLink = this._actionLinkList[i];
...
HY.Core.Action.Manager.prototype.removeActionInSpriteWithoutGroup = function(sprite,group,childRemoved){
	var i,j;
	for(i=this._actionLinkList.length-1;i>0;--i){
