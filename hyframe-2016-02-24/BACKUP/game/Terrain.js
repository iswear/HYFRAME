...
HY.Game.Terrain.prototype.defaultAnchorX = 0;
HY.Game.Terrain.prototype.defaultAnchorY = 0;
...
HY.Game.Terrain.prototype.defaultCacheEnable = true;
HY.Game.Terrain.prototype.initMember = function(config){
...
HY.Game.Terrain.prototype.onEnterFrame = function(pDeltaTime){
    ...
    if(i >= 0){
        ...
        this._recalculateObjects.splice(0,this._recalculateObjects.length);
...
HY.Game.Terrain.prototype.onPaint = function(pRc,pRect,isCacheRc){
	...
	if(this._natureLayer){
		if(this._natureLayer.backgroundBush && this._natureLayer.backgroundBush.length>0){
			var image = resourceManage.getImage(this._natureLayer.backgroundBush,null,null,null);
            if(image != null){
                var pattern = pRc.createPattern(image,"repeat");
                pRc.setFillStyle(pattern);
                pRc.fillRect((-this._anchorPixelX),(-this._anchorPixelY),this.getWidth(),this.getHeight());
            }
		}
		if(this._natureLayer.paths){
			...
			for(var i=0; i<len ; ++i){
				...
				if(curpath.pathBush && curpath.pathBush.length > 0){
					...
					if(linelen > 0){
                        var image = resourceManage.getImage(curpath.pathBush,null,null,null);
                        if(image != null){
                            pRc.beginPath();
                            for(var j=0; j<linelen ; ++j){
                                var curline = curpath.pathLines[j];
                                pRc.moveTo(curline.srcX,curline.srcY);
                                pRc.lineTo(curline.desX,curline.desY);
                            }
                            var pattern = pRc.createPattern(image,"repeat");
                            pRc.setStrokeStyle(pattern);
                            if(curpath.pathSmooth){
                                pRc.setLineCap("round");
                                var tempalpha = pRc.getGlobalAlpha();
                                for(var k = 0;k<5;k++){
                                    pRc.setLineWidth(curpath.pathWidth+100-9.375*k);
                                    pRc.setGlobalAlpha(globalAlpha);
                                    pRc.stroke();
                                }
                                pRc.setGlobalAlpha(tempalpha);
                            }else{
                                pRc.setLineWidth(curpath.pathWidth);
                                pRc.stroke();
                            }
                        }
					}
	...
	if(this._blockLayer && this._blockLayer.blocks){
		...
		for(;i>0;--i){
			...
			if(curblock.texture && curblock.coordinates && curblock.texture.src && curblock.coordinates.length > 0){
				var image = resourceManage.getImage(curblock.texture.src,null,null,null);
				if(image){
					...
					for(;j>0;--j){
						...
						pRc.drawImageExt(image,btSrcX,btSrcY,btSrcWidth,btSrcHeight,desX,desY,this._blockWidth,this._blockHeight);
					}
				}
			}
		}
