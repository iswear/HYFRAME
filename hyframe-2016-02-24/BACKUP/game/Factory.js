...
HY.Game.Factory.createModel = function(config){
	if(!config){
		config = {
			name:'新装配体'
		};
	}
	var curmodel = new HY.Game.Model(config);
	var childnodes = config.childNodes;
	if(childnodes){
		var len = childnodes.length;
		for(var i=0; i<len ; ++i){
			curmodel.addChildUnit(HY.Game.Factory.createUnit(childnodes[i]));
		}
	}
	return curmodel;
}
...
HY.Game.Factory.createEmptyModelJson = function(){
	var emptyModel = {
		name:'新装配体',
		x:200,
		y:150,
