...
HY.GUI.TreeView.prototype.reloadData = function(){
    ...
    if(this._root != null){
        this._nodeRoot = {layout:{y:0,height:0},nodePath:null,view:null,childNodes:[]};
...
HY.GUI.TreeView.prototype._treeCreate = function(dataSource , node, nodePathStr, layoutY){
    ...
    for(var i=0;i<childNodeNum;++i){
        var childNode = {layout:{y:0,height:0},view:null,childNodes:[]};
        var newNodePathStr = "";
        if(nodePathStr == ""){
            newNodePathStr = ""+i;
        }else{
...
HY.GUI.TreeView.prototype._setNodeView = function(nodePath, nodeView){
    if(nodePath){
        ...
        node.view = nodeView;
        nodeView.setX(0);
        nodeView.setY(node.layout.y);
        nodeView.setWidth(this.getContentWidth());
        nodeView.setHeight(node.layout.height);
...
HY.GUI.TreeView.prototype._ergodicNode = function(dataSource,node,offsetY,maxY){
    if(node.layout.y < maxY){
        if(node.layout.y + node.layout.height > offsetY && node.layout.height > 0){
            if(!node.view){
                var nodeView = dataSource.viewOfNodeInPath(this,node.nodePath);
                if(nodeView != null){
                    if(!nodeView.checkEventListener("mousedown",this.onNodeMouseDown,this)){
